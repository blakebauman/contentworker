...
automatically; production builds require an explicit token. Optional SSO: run `admin-bff` (included
in `docker compose up`, port **8790**) and set `VITE_SSO_LOGIN_URL=http://localhost:8790/auth/login`
plus the `OIDC_*` variables in `.env` (see [Configuration → Admin BFF](./configuration.md#admin-bff-oidc-sso)).
